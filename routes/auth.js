...
const sentverifymail = async (firstName, email, user_id) => {
  ...
  try {
    ...
    const mailoptions = {
      ...
      html: '<p> hii ' + firstName + ', please click to verify <a href="  https://sfddfsd.herokuapp.com/verify?id=' + user_id + '">verify</a>your mail</p>'
    }
...
authrouter.get("/verify", async (req, res) => {
  ...
  try {
    ...
    const update = await Auth.updateOne({ _id: req.query.id }, { $set: { isVarified: 1 } })
    console.log(update);
  ...
  try {
    ...
    cloudinary.uploader.upload(file.tempFilePath, async (err, result) => {
      ...
      const files = new Auth({
        ...
        password: spassword, cPassword: spassword,
...
authrouter.get("/user/:id", checkauth, async (req, res) => {
  ...
  try {
   ...
   data.password = undefined, data.cPassword = undefined, data.isVarified = undefined, data.isAdmin = undefined

   res.status(200).send({ status: "success", details: data })

 } catch (error) {

   res.status(400).send({ status: "Bad Request", details: "", message: "something wrong user not found in data base..." })
 }
})
...
authrouter.put("/user/update/:id", checkauth, async (req, res) => {
  ...
  try {
    ...
    const { firstName, lastName, email, phone, gender, age, address, state, zip, city } = req.body
    const file = req.files.image
    ...
    const data = {
      firstName, lastName, email, phone, gender, age, address, state, zip, city,
      image: result?.secure_url || user.image, cloudinary_id: result?.public_id || user.cloudinary_id,
    };

    user = await Auth.findByIdAndUpdate(req.params.id, data, { new: true });

    user.password = undefined, user.cPassword = undefined, user.isVarified = undefined, user.isAdmin = undefined

    res.status(200).send({ status: "success", updateDetails: user });

  } catch (error) {

    res.status(400).send({ status: "Bad Request", updateData: "", message: "something wrong user not found..." })
  }
})
...
authrouter.delete("/user/delete/:id",checkauth, async (req, res) => {
  ...
  try {
    ...
    res.status(400).send({ status: "Bad Request", deleteData: "", message: "user already deleted..." })
