  ...
  try {
    ...
    if (user) {
      const data = {
        ...
        "address": req.user.address, "city": req.user.city,
        "state": req.user.state, "zip": req.user.zip,
...
adminrouter.put("/updateAdmin/:id", [checkauth, adminauth], async (req, res) => {
  try {
    ...
    const data = {
      firstName: req.body.firstName || user.firstName, lastName: req.body.lastName || user.lastName,
      email: req.body.email || user.email, phone: req.body.phone || user.phone,
      gender: req.body.gender || user.gender, age: req.body.age || user.age,
      address: req.body.address || user.address, city: req.body.city || user.city,
      state: req.body.state || user.state, zip: req.body.zip || user.zip,
...
adminrouter.get("/allUser", [checkauth, adminauth], async (req, res) => {
  ...
  try {
    ...
    if (user) {
     ...
     return res.status(400).send({status:"Bad Request",data:"",message:"user not found"})
      }
      else if(data.length>=0) {
     ...
     res.status(400).send({status:"Bad Request",data:"",message:"something wrong..."})
  }
})
